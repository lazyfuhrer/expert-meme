...
class AppointmentReport:
    ...
    def payment_mode_summary(self, request):
        ...
        paginator = Paginator(payment_mode_list, 50)
    ...
    def appointments_per_doctor(self, request):
        appoinment = Appointment.objects.filter(
            ...
            name=Concat(F('doctor__first_name'), Value(' '
                                                            ), F('doctor__last_name'))
        ).annotate(
        ...
        page = request.GET.get('page', 1)
        paginator = Paginator(appoinment, 50)
    ...
    def invoiced_income_per_doctor(self, request):
        ...
        invoice_items = InvoiceItems.objects.filter(
            ...
            name=Concat(F('doctor__first_name'), Value(' '), F('doctor__last_name'), output_field=CharField())
        ).annotate(
            ...
            invoice=Sum('invoice__grand_total', default=0)
        ).order_by('-name')

        for i in invoice_items:
            i['cost'] = price_format(i['cost'])
            i['discounts'] = price_format(i['discounts'])
            i['income'] = price_format(i['income'])
            i['tax'] = price_format(i['tax'])
            i['invoice'] = price_format(i['invoice'])

        page = request.GET.get('page', 1)
        paginator = Paginator(invoice_items, 50)
    ...
    def get_income_per_procedure(self, request):
        ...
        for item in invoice_items_list:
            if '>' in item['procedure__name']:
                main_procedure_name = item['procedure__name'].split(' ')[0].strip()

            elif '-' in item['procedure__name']:
                main_procedure_name = item['procedure__name'].split(' ')[0].strip()
        ...
        for index, (procedure_name, aggregates) in enumerate(procedure_aggregates.items(), start=1):
            result_list.append({
                's.no.': index,
                'procedure': price_format(procedure_name),
    ...
    def get_appointment_per_procedure(self, request):
        ...
        .annotate(count=Count('procedure__name'))
    )
        procedure_aggregates = {}
    ...
    def get_advance_payments(self, request):
        ...
        ).values('Id','name', 'wallet', 'due'))

        total_wallet = price_format(sum(item['wallet'] for item in payment))
        total_due = price_format(sum(item['due'] for item in payment))
    
    # Append the total count as a dictionary to the list
        payment.insert(0,{'s.no.': "","Id":"Total", "wallet": total_wallet, "due": total_due})
    ...
    # Add serial number to each entry in the list
        ...
        # Create a new dictionary with 's.no' first
            ...
            invoice['wallet'] = price_format(invoice['wallet'])
    ...
    def get_appointment(self, request):
        ...
        page = request.GET.get('page', 1)
        paginator = Paginator(appointment, 50)
    ...
    def get_daily_appointments(self, request):
        ...
        page = request.GET.get('page', 1)
        paginator = Paginator(response_data, 50)
    ...
    def get_appointment_plans(self, request):
        ...
        data= [
            {'s.no': '1', 'procedure name':'chiropractic 1/12', 'total appointments':self.get_procedure_appointments_count('Chiropractic '
                                                                                                                         'Treatment Plan > '
                                                                                                                         'Session 1/12'), 'total earnings': price_format(self.get_procedure_appointments_earning('Chiropractic '
                                                                                                                         'Treatment Plan > '
                                                                                                                         'Session 1/12'))},
            {'s.no': '2', 'procedure name':'chiropractic 12/12', 'total appointments':self.get_procedure_appointments_count('Chiropractic '
                                                                                                                         'Treatment Plan > '
                                                                                                                         'Session 12/12'), 'total earnings': price_format(self.get_procedure_appointments_earning('Chiropractic '
                                                                                                                         'Treatment Plan > '
                                                                                                                         'Session 12/12'))},
            {'s.no': '3', 'procedure name':'chiropractic 1/20', 'total appointments':self.get_procedure_appointments_count('Chiropractic '
                                                                                                                         'Treatment Plan > '
                                                                                                                         'Session 1/20'), 'total earnings': price_format(self.get_procedure_appointments_earning('Chiropractic '
                                                                                                                         'Treatment Plan > '
                                                                                                                         'Session 1/20'))},
            {'s.no': '4', 'procedure name':'chiropractic 20/20', 'total appointments':self.get_procedure_appointments_count('Chiropractic '
                                                                                                                         'Treatment Plan > '
                                                                                                                         'Session 20/20'), 'total earnings': price_format(self.get_procedure_appointments_earning('Chiropractic '
                                                                                                                         'Treatment Plan > '
                                                                                                                         'Session 20/20'))},

            {'s.no': '5', 'procedure name':'physiotherapy 1/12', 'total appointments': physiotherapy_1_12_appointments, 'total earnings': price_format((self.get_procedure_appointments_earning('Physiotherapy Plus - 1/12'
                                                            ) + self.get_procedure_appointments_earning(
                                                                       'Physiotherapy Standard - 1/12')))},
            {'s.no': '6', 'procedure name':'physiotherapy 12/12', 'total appointments': physiotherapy_12_12_appointments, 'total earnings': price_format((self.get_procedure_appointments_earning('Physiotherapy Plus - 12/12'
                                                            ) + self.get_procedure_appointments_earning(
                                                                       'Physiotherapy Standard - 12/12')))},
            {'s.no': '7', 'procedure name':'physiotherapy 1/20', 'total appointments': physiotherapy_1_20_appointments, 'total earnings': price_format((self.get_procedure_appointments_earning('Physiotherapy Plus - 1/20'
                                                            ) + self.get_procedure_appointments_earning(
                                                                       'Physiotherapy Standard - 1/20')))},
            {'s.no': '8', 'procedure name':'physiotherapy 20/20', 'total appointments': physiotherapy_20_20_appointments, 'total earnings': price_format((self.get_procedure_appointments_earning('Physiotherapy Plus - 20/20'
                                                            ) + self.get_procedure_appointments_earning(
                                                                       'Physiotherapy Standard - 20/20')))},
            ]
        
        total_appointments = sum(item['total appointments'] for item in data if 'total appointments' in item)
        data.insert(0, {'s.no': '', 'procedure name': 'Total', 'total appointments': total_appointments, 'total earnings': ''})
        page = request.GET.get('page', 1)
        paginator = Paginator(data, 50)
    ...
    def get_daily_patient(self, request):
        ...
        paginator = Paginator(response_data, 50)
        page = request.GET.get('page', 1)
        paginated_result_list = paginator.get_page(page)
