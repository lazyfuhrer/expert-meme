...
def patient_booking_notifications(appointment_data):
    ...
    EmailUtils(patient.email, "Appointment Confirmation", "email/appointment_booked_by_patient", context_appointment).send()
    sms_appointment = SMSUtils('appointment_booked_by_patient')
    sms_appointment.send(patient.phone_number[-10:], context_appointment_phone)

    sms_appointment = SMSUtils('location_link')
    sms_appointment.send(patient.phone_number[-10:], context_appointment_location_phone)

    # Send payment confirmation notification
    EmailUtils(patient.email, "Payment Confirmation", "email/confirm-payment", context_payment).send()
    sms_payment = SMSUtils('confirm_payment')
...
def send_payment_notifications(user, clinic_id, payment_link):
        ...
        EmailUtils(user.email, "Payment Link", "email/appointment_payment_link", data).send()
        # sms = SMSUtils('confirm_payment')
        # sms.send(user.phone_number, data)
